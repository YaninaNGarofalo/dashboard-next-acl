...
import {collection, doc,getDoc, getDocs, addDoc, deleteDoc, getFirestore, ref, setDoc} from 'firebase/firestore'
...
export const addProject = async (project) => {
  ...
  try {
    console.log(project, userPath)
    const user =  await doc(db,"TR_Users/0")
    project['Modified By'] = user
    project['Created By'] = user
    // project['Account Manager'] = user
    const collectionID = uuidv4().replaceAll("-", "");
    const docRef =  setDoc(doc(db, "TR_Projects", collectionID), project) // await addDoc(collection(db, clientRef), client)
...
export const addClient = async (client, userPath) => {
  try {
    console.log(client, userPath)
    const user =  await doc(db,"TR_Users/0")
    client['Modified By'] = user
    client['Created By'] = user
    client['Account Manager'] = user
    const collectionID = uuidv4().replaceAll("-", "");
    const docRef =  setDoc(doc(db, "TR_Clients", collectionID), client) // await addDoc(collection(db, clientRef), client)
    console.log(docRef.id)    
    return true
  } catch (error) {
    console.log(error) 
