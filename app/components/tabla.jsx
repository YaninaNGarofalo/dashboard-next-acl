        ...
        <thead className="text-xs uppercase dark:bg-gray-700 dark:text-gray-400">
            <tr>
                {headers.map((header,i)=>{ 
                     if(header !='actionID'){
                        return <th scope="col" className="px-3 py-3 text-xs"  key={`${header}`} onClick={() => handleTableSorting(header)}> <span className="flex">  {header.toLocaleUpperCase()} <svg className="w-3 h- ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
